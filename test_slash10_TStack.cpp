...
int main()
{
    ...
    vector<double> window_width_2 = {0.0, 0.4, 0.8};
    ...
    // Draw the original profile
    profile_b_corr.Draw();
    profile_b_corr.GetXaxis()->SetTitle("Window Width");
    profile_b_corr.GetYaxis()->SetTitle("b_corr");
    profile_b_corr.SetMinimum(-1);
    profile_b_corr.SetMaximum(1);

    // Create a legend
    TLegend legend(0.7, 0.7, 0.9, 0.9);
    legend.AddEntry(&profile_b_corr, "Original", "l");

    // Draw the additional profiles
    for (int i = 0; i < window_width_2.size(); ++i)
    {
        TProfile profile_b_corr_2("profile_b_corr_2", "b_corr vs Window Width", window_width.size() - 2, &window_width[1]);
        for (int k = 0; k < window_width.size() - 2; ++k)
        {
            sum_nF_k += fwdMult[k];
            nFSquared_sum_k += fwdMult[k] * fwdMult[k];
            ...
            profile_b_corr_2.Fill(window_width[k + 1], b_corr);
        }

        profile_b_corr_2.SetLineColor(i + 2);
        profile_b_corr_2.Draw("same");

        // Add entry to the legend
        legend.AddEntry(&profile_b_corr_2, Form("Width %g", window_width_2[i]), "l");
    }
