...
{
    TChain *chain = new TChain("Event%d"); // Create a TChain object

    // Add the tuples to the TChain
    TFile *file = TFile::Open("pythia8MB_CR1_RHoff_1.root");
    if (file && !file->IsZombie()) {
        //char eventnumber[30];
        for (Int_t i = 1; i <= 999999; ++i) {
            //sprintf(eventnumber,"Event%d",i)
            TString tupleName = Form("Event%d", i);
            if (file->Get("Event%d",i)) {
    ...
    for (Long64_t iEvent = 0; iEvent < eventNumber; ++iEvent)
    {
        ...
        // Particle loop
        for (Int_t i = 0; i < chain->GetBranch("nParticles")->GetLeaf("nParticles")->GetValue(); ++i)
        {
